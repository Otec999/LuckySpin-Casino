...
class AdminController extends Controller
{
    ...
    public function createTournier(Request $r){
        ...
        return response(['success' => true, 'mess' => 'РЈСЃРїРµС€РЅРѕ' ]);
    ...
    public function changeWithdraw(Request $r){
        ...
        if($info->status != 0){
            return response(['success' => false, 'mess' => 'Р’С‹РїР»Р°С‚Р° РѕС‚РјРµРЅРµРЅР° РёР»Рё РѕС‚РїСЂР°РІР»РµРЅР°' ]);
        ...
        if($status == 1){
            ...
            if($info->ps == "Qiwi" || $info->ps == "VISA") {
                ...
                return response(['success' => true, 'mess' => 'Р’С‹РїР»Р°С‚Р° РїРѕСЃС‚Р°РІР»РµРЅР° РІ РѕР±СЂР°Р±РѕС‚РєСѓ' ]);
            }
    ...
    public function saveSetting(Request $r){
        ...
        return response()->json(['success' => true, 'mess' => 'РЈСЃРїРµС€РЅРѕ' ], 200);
    }


    public function deleteDepPromo(Request $r){
        if(\Auth::user()->admin == 2) return response()->json(['message' => 'РћС€РёР±РєР°'], 401);
        $id = $r->id;
    ...
    public function createDepPromo(Request $r){
        ...
        if($count_promo > 0){
            return response()->json(['message' => 'РўР°РєРѕР№ РїСЂРѕРјРѕРєРѕРґ СѓР¶Рµ РµСЃС‚СЊ' ], 401);
        }
        if($name == ''){
            return response()->json(['message' => 'РќРµ СѓРєР°Р·Р°РЅРѕ РЅР°Р·РІР°РЅРёРµ' ], 401);
        }
        if($sum < 1){
            return response()->json(['message' => '% РјРµРЅСЊС€Рµ 1' ], 401);
        }
        if($act < 1){
            return response()->json(['message' => 'РђРєС‚РёРІР°С†РёСЏ РјРµРЅСЊС€Рµ 1' ], 401);
        }

        if(\Auth::user()->admin == 2 && $sum > 30) return response()->json(['message' => 'РњР°РєСЃРёРјР°Р»СЊРЅС‹Р№ РїСЂРѕС†РµРЅС‚ 30'], 401);
    ...
    public function createPromo(Request $r){
        ...
        if($sum < 1){
            return response()->json(['message' => 'РЎСѓРјРјР° РјРµРЅСЊС€Рµ 1' ], 401);
        }
        if($act < 1){
            return response()->json(['message' => 'РђРєС‚РёРІР°С†РёСЏ РјРµРЅСЊС€Рµ 1' ], 401);
        }  

        if(\Auth::user()->admin == 2 && $sum > 50) return response()->json(['message' => 'РњР°РєСЃРёРјР°Р»СЊРЅР°СЏ СЃСѓРјРјР° 50'], 401);
...
public function addRandom(Request $r){
    ...
    if($count > 0){
        return response(['success' => false, 'mess' => 'РўР°РєРѕР№ РєР»СЋС‡ СѓР¶Рµ РµСЃС‚СЊ' ]);
    }
...
public function addStatus(Request $r){
    ...
    if($count > 0){
        return response(['success' => false, 'mess' => 'РўР°РєРѕР№ СЃС‚Р°С‚СѓСЃ СѓР¶Рµ РµСЃС‚СЊ' ]);
    }
    Status::create(array(
        'bonus' => $bonus,
        'deposit' => $deposit,
...
//     $user->save();
//     return response(['success' => true,'type'=> 'success', 'mess' => "РЈСЃРїРµС€РЅРѕ" ]);
